...
RegisterNetEvent("need_boombox:place_boombox")
AddEventHandler("need_boombox:place_boombox", function()
	...
	local objectCoords = (coords + forward * 1.0)
        ...
        TriggerServerEvent("need_boombox:set_boombox", boomBoxName, objectCoords)
    end)
end)
...
function pickupBoomBox(coords)
    if DoesEntityExist(currentData) then
        ...
        if not DoesEntityExist(currentData) then
            TriggerServerEvent("need_boombox:remove_boombox", coords, boomBoxName)
            currentData = nil
        end
        boomBoxName = nil
        boomBoxOwner = "nil"
    ...
    ESX.UI.Menu.Open("dialog", GetCurrentResourceName(), "setvolume",
        {
            ...
            if value < 0 or value > 1 then
                ...
                TriggerServerEvent("need_boombox:set_volume", boomBoxName, value)
                menu.close()
            end
        end, function(data, menu)
            menu.close()
    ...
    ESX.UI.Menu.Open("dialog", GetCurrentResourceName(), "play",
        {
            ...
            TriggerServerEvent("need_boombox:play_music", boomBoxName, data.value, 1, objCoords)
            menu.close()
        end, function(data, menu)
            menu.close()
        end)
...
function stop(coords)
            ...
            if(distance < 50) then
                TriggerServerEvent("need_boombox:stop_music", boomBoxName)
            else
...
RegisterNetEvent('need_boombox:boomboxes_menu')
AddEventHandler('need_boombox:boomboxes_menu', function(data) OpenAdminMenu(data); end)