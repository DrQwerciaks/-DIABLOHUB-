...
Citizen.CreateThread(function()
	while true do
		...
		if otwiera then
			...
			-- Instrukcja
			local coords = GetEntityCoords(GetPlayerPed(-1))
			ESX.Game.Utils.DrawText3D(coords, "[~g~G~s~] - PODNIEŚ ZAPADKĘ~n~[~g~H~s~] - ZABLOKUJ ZAPADKĘ~n~[~g~X~s~] - ANULUJ", 0.6)
	...
	while true do
		...
		if otwiera then
			...
			elseif pin1 and pin2 and pin3 and pin4 then
				...
				DisableControlAction(0, 73, false)
				ESX.ShowNotification('Udało Ci się otworzyć zamek!')
