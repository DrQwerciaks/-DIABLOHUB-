...
function MFD:InputHandler(plant)
...
function MFD:TextHandler()
  ...
  if (plyData.job and plyData.job.label and plyData.job.label == self.PoliceJobLabel) then
    self.PolText = { closest = closest, strA = "[ Naciśnij [~r~ E ~s~] aby zniszczyć ]" }
  end
...
function MFD:DrawText3D(x,y,z, text)
  ...
  if onScreen then
    ...
    AddTextComponentString("<font face='Poppins-Medium' size= '15'>".. text .."</font>" ) 
