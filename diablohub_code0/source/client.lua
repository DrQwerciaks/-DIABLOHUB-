	...
	while true do
		...
		if IsControlJustPressed(1, Keys['F7']) then
			if ESX.GetPlayerData().job.name == 'police' then
				...
				if not IsPedBeingStunned(playerPed) and not IsEntityPlayingAnim(playerPed, "random@mugging3", "handsup_standing_base", 3) then
					ESX.TriggerServerCallback("Panicbutton:sprawdz",function(item)
						if item > 0 then
							local ko = GetEntityCoords(playerPed, true)
							if wtrc == false then
								wtrc = true
								TriggerServerEvent('panicbutton:on', ko)
								TriggerEvent('koniec:pb')
								ExecuteCommand("me Użył Panic Buttona")
							else
								ESX.ShowNotification('~r~Odczekaj zanim znowu użyjesz panic buttona!')
							end
						else
							ESX.ShowNotification("~r~Nie masz Panic Buttona")
						end
					end)
				else
					ESX.ShowNotification("~r~Nie możesz użyć mając rece w górze")
				end
			end
		end
	end
...
AddEventHandler('panicbutton:on', function(target, coords, name)
	-- PlaySoundFrontend(-1, "HACKING_FAILURE", 0, 1)
	-- ESX.ShowNotification("CODE0 | Trasa do zgloszenia policjanta zostala wyznaczona")
	...
	-- AddTextComponentString("<font face='Poppins-Medium' size= '11'>Code 0!</font>" )
